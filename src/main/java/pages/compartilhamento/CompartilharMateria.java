...
public class CompartilharMateria extends AceitacaoAbstractTest {
	private String urlDaPagina;
	private String urlAposClicar;
	
	public CompartilharMateria() throws Exception {
		super();
	}
	
	public CompartilharMateria(Navegador navegador) throws Exception {
		super(navegador);
	}
	
	public CompartilharMateria(Navegador navegador, Device device) throws Exception {
	...
	public void clicarNoGMaisUmGoogle() {
		String iDIframe = getDriver().findElement(By.id("googlePlus")).findElement(By.tagName("iframe")).getAttribute("id");
		
		getDriver().switchTo().frame(iDIframe);
		getDriver().findElement(By.className("dIa")).click();
		
	}

	public boolean abriuUmaNovaJanela() {
		this.urlAposClicar = getDriver().getCurrentUrl();
