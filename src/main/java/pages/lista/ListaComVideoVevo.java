...
public class ListaComVideoVevo extends ListaComVideoGlobo {
	
	public ListaComVideoVevo(Navegador navegador) throws Exception {
		super(navegador);
	}
	
	public boolean exibiuTituloDescricaoContadorVideoDoVevoComParaCadaFato() throws InterruptedException{
		...
		for(int i=0; i < totalDeItensDalistaDeFatos(); i++){
			...
			WebElement videoVevo = listaComTodosOsFatos().get(i).findElement(By.className("video")).findElement(By.tagName("iframe"));
			String urlVideoVevo = videoVevo.getAttribute("src");
			...
			if(!urlVideoVevo.contains("vevo.com") || !videoVevo.isDisplayed()){
				imprimirMensagemDeErro("vídeo do vevo", i);
				return false;
			}
