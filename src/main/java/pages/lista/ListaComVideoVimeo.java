...
public class ListaComVideoVimeo extends ListaComVideoGlobo {
	
	public ListaComVideoVimeo(Navegador navegador) throws Exception {
		super(navegador);
	}
	
	public boolean exibiuTituloDescricaoContadorVideoDoVimeoComParaCadaFato() throws InterruptedException{
		...
		for(int i=0; i < totalDeItensDalistaDeFatos(); i++){
			...
			WebElement videoVimeo = listaComTodosOsFatos().get(i).findElement(By.className("video")).findElement(By.tagName("iframe"));
			String urlVideoVimeo = videoVimeo.getAttribute("src");
			...
			if(!urlVideoVimeo.contains("vimeo.com") || !videoVimeo.isDisplayed()){
				imprimirMensagemDeErro("vídeo do Vimeo", i);
				return false;
			}
